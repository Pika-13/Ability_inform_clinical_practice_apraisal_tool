#Author: Joana RR
#Starting date: January 16th, 2025
...
database_v0 <- read_csv("Final_phase_Article/database/articles_167.csv") %>%
  rename_with(~ str_remove(., "^.*?-"))  %>% 
  mutate(across(where(is.character),
                ~ str_remove(., '^\\[\\{"text":"') %>%
                  str_remove('"\\}\\]$'))) %>% 
  select(key, authors, title,year,doi,
         RCT_aim_trial,
         RCT_design_aim,
...
database_vapril27 <- database_v0 %>%
  ...
  select(key, authors, title, year,doi,
         ...
         outcome)
#save excel
write_csv(database_v1, "Final_phase_Article/database/database_covidence_v1.csv")
write_xlsx(database_v1, "Final_phase_Article/database/database_covidence_v1.xlsx")

#save database_vapril27 = correction
write_csv(database_vapril27, "Final_phase_Article/database/database_vapril27.csv")
write_xlsx(database_vapril27, "Final_phase_Article/database/database_vapril27.xlsx")
